...
def save_pdf(filename, patient_name, document_text, document_date=None, include_date=False):
    ...
    pdf.cell(0, 20, txt = f"RECEITA MÉDICA", ln = True, align = 'C')
    pdf.cell(0, 20, txt = f"Nome: {patient_name}", ln = True, align = 'L')
    pdf.ln(10)
    pdf.multi_cell(0, 10, txt = document_text)
    if include_date and document_date is not None:
        pdf.ln(30)
        pdf.cell(0, 10, txt = f"{document_date.strftime('%d/%m/%Y')}", ln = True, align = 'C')
    pdf.output(filename)
